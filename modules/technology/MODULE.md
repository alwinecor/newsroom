...
- module：`technology`；状态：enabled。
- 每周通常筛选 7 条具有国际意义的重要新闻，允许轻度浮动；schema 校验范围为 5–8 条。
- 本文件独立定义本模块的内容要求；执行前读取同目录 `schema.json`。期数、共享窗口和发布流程由 `PIPELINE.md` 定义。

## 2. 内容范围

重点：AI、foundation models、developer tools、major software/platform changes、semiconductors、major open-source projects、important research releases。

建议来源：OpenAI、Anthropic、Google / DeepMind、Microsoft、Meta、NVIDIA、AMD、TSMC、ASML、Apple、major GitHub projects、arXiv / research institutions、Reuters / FT / TechCrunch 等。不要要求每周所有来源都出现。优先使用原始公告的 URL；若只有可靠媒体报道，可使用下述 Tier B 媒体。

## 3. 来源与核实

- Tier A（source_level: primary）：公司官方公告、官方博客、GitHub 官方 repository / release、官方研究机构，以及相关政府和国际组织。
...
- 输出路径：`data/issues/YYYY-MM-DD/technology.json`，不添加 Markdown 代码围栏。
- 顶层必须包含 module（固定为 `technology`）、issue_date、window、items；issue_date 与目录日期一致，window 与本期其他模块一致。
- 每条 item 必须包含下表全部字段。字段类型、数量和额外字段限制以同目录 schema.json 为准，不得自行增添字段。

| 字段 | 要求 |
| --- | --- |
| id | 模块内唯一，建议 `technology-001` 等 |
| category | 具体主题分类 |
